    ...
    def __init__(self):
        ...
        if XLDEMO_HUGGINGFACE_ACCESS_TOKEN is not None and XLDEMO_HUGGINGFACE_ACCESS_TOKEN.strip() != '':
            ...
            self.pipe = DiffusionPipeline.from_pretrained(
                self.model_key_base, torch_dtype=torch.float16, use_safetensors=True, use_auth_token=access_token)
            ...
            if self.load_refiner_on_startup:
                ...
                self.pipe_refiner = DiffusionPipeline.from_pretrained(
                    self.model_key_refiner, torch_dtype=torch.float16, use_safetensors=True, use_auth_token=access_token)

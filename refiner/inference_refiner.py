...
def inference(seed=-1):
    ...
    images = pipe(prompt=[prompt] * num_images, negative_prompt=[negative_prompt] * num_images, guidance_scale=guidance_scale,
                  num_inference_steps=num_inference_steps, generator=generators).images
    ...
    for i, img in enumerate(images):
        img.save(
            f'inference-{int(time_.time())}-{str(i+1).zfill(2)}-{seed+i}.png')
    ...
    for i, img in enumerate(images):
        img.save(
            f'inference-{int(time_.time())}-{str(i+1).zfill(2)}-{seed+i}-refiner.png')
